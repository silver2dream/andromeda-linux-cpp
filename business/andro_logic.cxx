...
typedef bool(CLogic::*handler)
	(lp_connection_t conn_ptr, lp_message_header_t msg_header, char *pkg_body, unsigned short pkg_body_len);
...
void CLogic::ThreadRecvProcFunc(char *msg_bffer) {
  auto msg_header_ptr = (lp_message_header_t) (msg_bffer);
  auto pkg_header_ptr = (lp_packet_header_t) (msg_bffer + ANDRO_MSG_HEADER_LEN);
...
void CLogic::HeartBeatTimeoutChecking(lp_message_header_t msg_header_ptr, time_t cur_time) {
  if (msg_header_ptr->sequence == msg_header_ptr->conn_ptr->sequence) {
	lp_connection_t conn_ptr = msg_header_ptr->conn_ptr;

	if (timeout_kick_enable == 1) {
	  kick(conn_ptr);
	} else if ((cur_time - conn_ptr->last_ping_time) > ANDRO_CONF_SOCK_TIMEOUT_KICK_DEFAULT_VALUE) {
	  kick(conn_ptr);
	}

	CMemory::FreeMemory(msg_header_ptr);
  } else {
	CMemory::FreeMemory(msg_header_ptr);
  }
}

bool CLogic::HandlePing(lp_connection_t conn_ptr,
  ...
  conn_ptr->last_ping_time = time(nullptr);
  SendNoBodyPkgToClient(msg_header, ANDRO_BUSINESS_CMD_PING);
  log_stderr(0, "received ping successful");
  return true;
}

void CLogic::SendNoBodyPkgToClient(lp_message_header_t msg_header, unsigned short msg_code) {
  auto send_buffer = (char *) CMemory::AllocMemory(ANDRO_MSG_HEADER_LEN + ANDRO_PKG_HEADER_LEN, false);
  auto send_info = send_buffer;
  memcpy(send_info, msg_header, ANDRO_MSG_HEADER_LEN);
  send_info += ANDRO_MSG_HEADER_LEN;

  auto pkg_header_ptr = (lp_packet_header_t) send_info;
  pkg_header_ptr->msg_code = htons(msg_code);
  pkg_header_ptr->pkg_len = htons(ANDRO_PKG_HEADER_LEN);
  pkg_header_ptr->crc32 = 0;
  msg_send(send_buffer);
}

bool CLogic::HandleRegister(lp_connection_t conn_ptr,
							lp_message_header_t msg_header,
							char *pkg_body,
							unsigned short pkg_body_len) {
  if (pkg_body == nullptr) {
...
bool CLogic::HandleLogin(lp_connection_t conn_ptr,
						 lp_message_header_t msg_header,
						 char *pkg_body,
						 unsigned short pkg_body_len) {
  return true;
